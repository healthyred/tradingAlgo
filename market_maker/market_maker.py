...
class OrderManager:
    ...
    def place_orders(self):
        ...
        ##Godwin Algorithm:
        # We place a long order and a stop loss, and upon hitting this we have a reversal
        # We update stoploss with a trend line each time
        # priceLong = -settings.ORDER_PAIRS[0]
        # priceShort = settings.ORDER_PAIRS[0]
        # if not self.long_position_limit_exceeded():
        #     buy_orders.append(self.prepare_order(priceLong))
        # if not self.short_position_limit_exceeded():
        #     sell_orders.append(self.prepare_order(priceShort))
